      ...
      <div className="prf-wall">
        {PCX_QUOTES.map((t, i) => (
          <figure className="prf-tcard" key={i}>
            ...
            <figcaption>
              {/* Org logo reuses the Consulting section's .cvc-logo treatment verbatim — same
                  sizing, same object-fit, same greyscale-free rendering — rather than a new
                  logo pattern. Also closes Prod's finding C1 (BM-402), which was that this
                  wall reads flat next to the quote cards on Case Studies and Pricing. */}
              {t.logo && <img className="cvc-logo prf-tlogo" src={t.logo} alt={t.org} loading="lazy" />}
