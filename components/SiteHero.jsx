...
const HeroCTAs = ({
  ...
  // rendered label always comes from trialCta(). The CURRENT label is in the list so that
  // passing it explicitly still resolves the href; without it the label would render but
  // primaryHref would silently stay /contact. "Try Dalgo for Free" stays as a retired label
  // (renamed to "Try Platform for Free", 15 Sep) so any page still passing it keeps working.
  if (['Try the Platform', 'Start Free Trial', 'Try Platform for Free', 'Try Dalgo for Free'].indexOf(primaryLabel) > -1 && window.trialCta) { const t = window.trialCta(); primaryLabel = t.label; primaryHref = t.href; }
