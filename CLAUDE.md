...
- **Consistent hero heights (no cross-page jump).** Two tiers via `min-height` on `.cvh-grid`: visual heroes (`.cvh-grid`, have a right-hand `.cvh-figure`) share one floor and centre their content; text-only heroes (`.cvh-grid-solo`) share a shorter floor and top-align. Every page within a tier is the exact same hero height, so switching nav tabs never shifts the fold. Don't set per-page hero heights.
